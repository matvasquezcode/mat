...
// ─── PERSISTENCE ────────────────────────────────────────────────────────────
const STORAGE_KEY = "curiosity_progress_v1";
...
async function generateLesson(topicLabel, topicId, hint = "") {
  try {
    ...
    console.warn(`generateLesson: falling back to mock lesson for "${topicLabel}"`, err);
    return mockLesson(topicLabel, hint);
  }
}

function mockLesson(topicLabel, hint) {
  return {
    title: `A Surprising Look at ${topicLabel}`,
    content: `This is a placeholder lesson about **${topicLabel}**. It demonstrates how a [glossary](a list of terms with definitions) term and a [heuristic](a mental shortcut for quick decisions) can be highlighted and tapped for definitions.${hint ? ` (Angle: ${hint})` : ""}`,
    fact: `💡 Did you know? This is mock content shown because the AI lesson service is unavailable.`,
    quiz: {
      question: `What is this lesson a placeholder for?`,
      options: [topicLabel, "Cooking", "Astronomy", "Sports"],
      answer: 0,
    },
    depth: [
      { heading: "Why this is mock content", body: "The app couldn't reach the lesson-generation backend, so a sample lesson is shown instead. Real lessons are generated by Claude based on the topic." },
      { heading: "Next steps", body: "Wire up a backend endpoint at /api/generate-lesson that calls the Anthropic API with your API key and returns the lesson JSON." },
      { heading: "Everyday relevance", body: "Mock data like this lets you build and test the UI before the backend integration is ready." },
    ],
  };
}
...
// ─── PRIMARY BUTTON ─────────────────────────────────────────────────────────
function PrimaryButton({ color = T.purple, onClick, children, style, ...props }) {
  return (
    <button
      onClick={onClick}
      style={{ width: "100%", background: `linear-gradient(135deg,${color},#6366f1)`, border: "none", borderRadius: 16, padding: "16px", color: "#fff", fontWeight: 900, fontSize: 16, cursor: "pointer", fontFamily: "inherit", ...style }}
      {...props}
    >
      {children}
    </button>
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "'Inter',-apple-system,sans-serif", paddingBottom: 60 }}>
      ...
      <div style={{ padding: "20px 18px" }}>
        ...
        <PrimaryButton color={topic.color} onClick={onQuiz} style={{ marginTop: 22 }}>
          Take the quiz →
        </PrimaryButton>
      </div>
    </div>
        ...
        {!submitted ? (
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
              ...
              <PrimaryButton color={topic.color} onClick={() => onDone("done", correct)} style={{ flex: 1, padding: "14px", fontSize: 14 }}>Continue →</PrimaryButton>
            </div>
          </>
        )}
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "'Inter',-apple-system,sans-serif", paddingBottom: 80 }}>
      ...
      <div style={{ padding: "20px 18px" }}>
        ...
        <PrimaryButton color={topic.color} onClick={onDone}>Complete lesson →</PrimaryButton>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  // xp / completion
  const [xpByTopic, setXpByTopic]     = useState(() => saved?.xpByTopic ?? Object.fromEntries(TOPICS.map(t => [t.id, 0])));
  const [completedByTopic, setCompletedByTopic] = useState(() => saved?.completedByTopic ?? Object.fromEntries(TOPICS.map(t => [t.id, 0]))); // count of completions
  const totalXp = Object.values(xpByTopic).reduce((a, b) => a + b, 0);

  // Streak: bump once per calendar day, reset if a day was missed
  const [streak] = useState(() => {
    const prevDate = saved?.lastActiveDate;
    const prevStreak = saved?.streak ?? 0;
    if (!prevDate) return 1;
    const dayMs = 86400000;
    const gap = Math.round((new Date(todayStr()) - new Date(prevDate)) / dayMs);
    if (gap === 0) return prevStreak || 1;
  ...
  // Re-fetch a daily lesson that failed to load
  ...
  const handleQuizDone = (action, correct) => {
    ...
    if (action === "depth")   { setScreen("depth"); return; }
  ...
  if (screen === "lesson") {
    ...
    if (lesson === "loading" || lesson == null) {
      return <LoadingOrErrorLesson topic={activeTopic} isError={false} onBack={() => setScreen("home")} />;
    }
    if (lesson === "error") {
      const onRetry = activeLesson === "error" ? () => generateExtra(activeTopic) : () => retryDailyLesson(activeTopic.id);
      return <LoadingOrErrorLesson topic={activeTopic} isError={true} onRetry={onRetry} onBack={() => setScreen("home")} />;
    }
  ...
  if (screen === "result") return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "'Inter',-apple-system,sans-serif", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 20px", textAlign: "center" }}>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 14, width: "100%", maxWidth: 380 }}>
        ...
        <PrimaryButton color={activeTopic?.color || T.purple} onClick={() => setScreen("home")} style={{ flex: 2, padding: "14px", fontSize: 15, borderRadius: 14 }}>Back to Home</PrimaryButton>
      </div>
    </div>
  );
